...
def main(argv=None):
    ...
    # The module objects only ever hold the CURRENT event, so a multi-event run has to grab
    # each pair as it is produced -- otherwise --out would silently write only the last one.
    pairs = []
    if a.out:
    ...
    if a.out:
        ...
        if len(pairs) == 1:
            b, j, sc = pairs[0]
            np.savez_compressed(a.out, arr=b, arr_jet=j,
            ...
            for i, (b, j, sc) in enumerate(pairs):
                f = f"{stem}_ev{i}{ext or '.npz'}"
                np.savez_compressed(f, arr=b, arr_jet=j,
                                    src=(sc if sc is not None else np.zeros(0, np.float32)),
                                    tau=bg.g.tau_grid())
            print(f"wrote {len(pairs)} files: {stem}_ev0{ext or '.npz'} .. "
                  f"{stem}_ev{len(pairs) - 1}{ext or '.npz'}")
            print("  (for a real dataset use fast_data.writer.FnoH5Writer, which streams one "
                  "event at a time instead of holding them all)")
